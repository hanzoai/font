...
`ZenSans` is exported from `zen/font/sans`, `ZenMono` can be found in `zen/font/mono`, and Zen Pixel variants are available from `zen/font/pixel`. All are `NextFontWithVariable` instances. You can learn more by [reading the `next/font` docs](https://nextjs.org/docs/app/building-your-application/optimizing/fonts).
...
```jsx
import { ZenSans } from "zen/font/sans";
...
```jsx
import { ZenSans } from "zen/font/sans";
import { ZenMono } from "zen/font/mono";
import { ZenPixelSquare } from "zen/font/pixel";
